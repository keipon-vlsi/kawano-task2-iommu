...
### 5.1 実行コマンド（検証済み・段階別 PPA ＋ GDS）
**CTS まで（さらに配線も）やるのを推奨**：配置だけだとクロック網が理想（skew=0）なので Fmax を
楽観し、配線寄生も入らない。CTS＋配線まで通すと現実的になる。`syn/run_pnr.sh` は
**配置→repair→CTS→グローバル配線**を一気に流し、**各段の PPA**と **GDS** を出す。
```bash
export PDK_ROOT=$PWD/pdk
.venv/bin/python syn/synth_osic.py full        # 1) 合成 → syn/build/full_netlist.v
bash syn/run_pnr.sh full 2.5 16                # 2) 段階別 P&R（引数: name period maxfanout）
# DETAILED=1 bash syn/run_pnr.sh full 2.5 16   #   detailed_route まで（遅い・実配線/GDS）
cat results/full_pnr.json                       # 各段の Fmax/面積/電力
```
実測（Full, hd, tt）：

| 段階 | Fmax | die 面積 | 電力 |
|---|---|---|---|
| 配置+repair | 60.2 MHz | 530,988 µm² @37% | 0.47 W |
| CTS 後 | 60.1 MHz | 561,624 µm² @40% | 0.60 W |
| グローバル配線後 | 59.4 MHz | 561,624 µm² @40% | 0.60 W |

合成のみ 19.5 MHz → 配置 repair で 60 MHz（fanout466 がバッファツリー化）→ CTS/配線で寄生ぶん微減。
**依然 400 MHz 未達＝経路が挿入バッファの長い連鎖＝本質はパイプライン化**。

### 5.1b GDS 出力と klayout 確認
`run_pnr.sh` が magic で **`results/<name>.gds`** を生成（std セル GDS を読んで実形状で stream）。
```bash
...
### 5.1c クリティカルパス経路の詳細の見方
`results/full_pnr.txt`（および合成のみは `results/full_sta.txt`）に OpenSTA の
`report_checks -path_delay max` 出力がある。読み方：
- `Startpoint:` 起点FF／`Endpoint:` 終点FF（このFF間の組合せ経路が律速）。
- 表の各行 = 経路上の**セルごとの遅延**：`Fanout / Cap / Slew / Delay / Time / セル名`。
  **Delay 列が大きい行＝ボトルネック**（例：合成のみは fanout 466 の `nor4` が 30.9 ns）。
...
```bash
.venv/bin/python syn/ppa_compare.py full        # -> results/ppa_compare.md
```
