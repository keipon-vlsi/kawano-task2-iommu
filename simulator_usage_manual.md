...
### F.1b コールドスタート / warm-up の読み方
`run.py` の出力に **`-- cold-start / warm-up --`** セクションがある：
- **`time to steady-state`** … 最初の到着から「定常に落ち着く」までのサイクル数（と ns、件数）。
  定義：レイテンシ分布は定常でも二峰性（大半は IOTLB ヒット、~1/8 がウォーク）なので、**定常 tail(p99) の
  1.5倍を超えるレイテンシ＝バックログで待たされたコールドスタート要求**とみなし、その最後の完了までを warm-up と数える。
  実体は「最初のフル2Dウォークでキャッシュが温まる時間 ＋ その間に積もった初期バックログが捌けるまで」。
- **`--measure peaks`** では末尾に **steady vs cold-incl の必要HW比較表**が出る：
  - `steady` 列＝温まった後の定常要求（3c/3d）。
  - `cold-incl` 列＝**コールドスタートから stall ゼロで wire rate を満たすのに要る HW**（cold-start のバッファ・
    ウォーカーのピークを含む真ピーク）。例（cached nested）：N 2→5、buffer 13→33、io_bridge 13→33。
  - 使い分け：**定常設計は steady、起動時から落とさない設計は cold-incl** を provision（design_premises §12：
    過渡は上流バッファ/ウォームアップで吸う、という方針なら steady＋上流で可）。
