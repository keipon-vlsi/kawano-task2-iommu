...
1. **同じ CAM 型 IOTLB でも、RTL の書き方（論理構造）で回路も周波数も大きく変わる**
   — 格納・16 比較器は同一、match→SPA の縮約論理だけ変えて **246 → 368 MHz（+50%）**。
...
|---|---|---|---|---|---|
| **T5** CAM + 優先エンコーダ | 16 比較器 → **直列優先鎖**（下位 OR）+ 16:1 mux | 246 | 53,466 | 10 | 素の CAM。優先鎖が深い |
| **T8** CAM + mux カスケード | 優先を **2:1 mux 16 段直列**に埋込 | 284 | 53,787 | 11 | 最深・最大。埋めても遅い |
| **T6** CAM + one-hot | one-hot 前提 → 優先廃止 → **AND-OR 平衡ツリー** | **368** | 52,874 | 7 | CAM の最速形（+50% vs T5） |
