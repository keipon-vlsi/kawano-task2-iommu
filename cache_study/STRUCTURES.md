...
- **整列制約（何整列か）**：T1 が 16 エントリをフル活用できる条件＝「同時に載る 16 ページが**16 ページ整列
  ブロック**に収まる」。各空間で：ページ番号 **16 ページ整列** / IOVA **64 KB 整列**（16×4KB）/ PTE 上
  **128 B 整列**（16 PTE×8B＝連続 2 本の 64B PTE ライン）。これらは同じ条件の別表現。
- **T1 が使える時**：(1) DMA バッファが 64 KB 整列で割り付けられ、ブロック境界に揃ってアクセス/fill が
  来る、(2) 同時 working set が 1 ブロック（16 ページ）以内で境界跨ぎの両側同時保持が不要、(3) superpage
  （2 MB ページなら 1 エントリで 512×4KB を覆い 4KB を 16 個並べる必要が消える）、(4) プリフェッチ等で
  ブロック境界に合わせ window を切替できる設計。**使えない**：任意オフセットの連続ストリーム（本タスクの
  一般形）。理由＝**coalescing が保証するのは 64B ライン＝8 整列まで**で、T1 の要求する 128B 整列は保証
  されない。→ 本タスクは T0/T7、T1 は上記の整列保証がある特殊条件でのみ。
...
- **lookup**：**27b 比較器 16 個並列**（最多）。16 入力 OR で hit。SPA は **lowest-index-wins の
  優先選択**（RTL は `for i=15..0: if(match[i]) lk_spa=spa[i]` の累算ループ）。
- **クリティカルパス**：`clkinvlp → a221oi → nand4 → or3 → o21ai → o211ai → a311o → a2111oi →
  o21ai → a31oi`（10 段）。比較は並列でも**優先選択コーンが深い**。任意アクセスに強いが最大・遅い＝
  構造化変種の削減対象基準。
- **重要（T8 との関係）**：T5 と T8 は**論理的に同一**（同じ優先選択）。yosys 抽象も**完全に同じ
  16 $mux + 31 $eq**（確認済み）＝**「優先エンコーダ」と「mux カスケード」は別構造ではなく、同じ優先
  mux カスケードの 2 通りの書き方**。差（246 vs 284 MHz）は abc のマッピングが落とす局所解の違い（後述 T8）。
  ※以前「T5＝優先エンコーダ＋mux ツリー」と分けて書いたのは不正確で、本注記が正。
...
- **クリティカルパス**：`clkinvlp → a2111oi → nand4 → nor4 → a22o → a221oi → nand4`（**段数 10→7**）。
  比較器・DFF は T5 と同（面積ほぼ同）だが**優先選択を消し +50% Fmax**。FA を採るなら T5/T8 でなく T6。
- **ここが唯一の "真の構造差"**：T5↔T8 は同じ優先 mux の書き方違い（246↔284, abc 感度）。対して
  T6↔(T5/T8) は **優先論理の有無**という本質的な違い（284→368）。速さの源は「優先の書き方」ではなく
  **「one-hot 前提で優先論理ごと消す」**こと。
- **ゲートレベル図**：`figs/figures/t5_t6_gates.png`（優先あり=直列依存 vs T6 の one-hot 並列 OR ツリーを
  対比。注：図の「priority encoder」は概念図で、実体は T5/T8 共通の優先 mux カスケード）。

## T8 — 優先選択を「前向きネスト三項」で書いた版（= T5 と同一論理）  284 MHz / 53,787µm² / DFF1228 / 段数11
- **着想**：T5 と**同じ lowest-index-wins 優先選択**を、`match[0]?spa[0]:match[1]?spa[1]:…:0` の
  **前向きネスト三項**で書いたもの（見かけは 2:1 mux の 16 段カスケード）。
- **重要事実**：**yosys 抽象ネットリストは T5 と完全に同一（16 $mux + 31 $eq、確認済み）**。`for+if` も
  ネスト三項も yosys は同じ「16 個の 2:1 mux 優先カスケード（$procmux）」に変換する。**T5 と T8 は別構造
  ではなく、同じ優先 mux の 2 通りの HDL コーディング**。
- **なぜ PPA が違うのか**：抽象が同じでも、カスケードの順序・信号名が両者で微妙に異なり、**abc の
  テクノロジマッピングが別の局所解に着地**する → T5 段数10/4089cell/246MHz、T8 段数11/4304cell/284MHz。
- **クリティカルパス**：`clkinv→o2111ai→nor4→and4→a221oi→o21ai→a22oi→o21ai→a21oi→o311a→o21ai`（11 段）。
- **観察1（段数 ≠ Fmax）**：T8 は段数が T5 より深い(11>10)のに **post-place Fmax は速い(284>246)**。
  fanout の差（低 fanout カスケード寄りのマッピングはリサイザが効く）。
- **観察2（メッセージ① の最純粋形）**：**同一論理を 2 通りに書くだけで Fmax 246↔284（+15%）**。「RTL の
  書き方で回路も周波数も変わる」を、論理を一切変えずに示す例。ただし両者とも T6(368) には届かない＝
  **優先論理を持つ限り勝てない；速さの源は優先の "書き方" ではなく "除去"（one-hot, T6）**。
