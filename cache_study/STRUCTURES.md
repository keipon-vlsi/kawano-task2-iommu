...
- **⚠ 整列前提・境界跨ぎで実効容量低下**：base 1 個のみ。fill で `VPN[26:4]` が変わると window 全体を
  クリアして再ベースするため、**16 エントリをフルに使えるのは 16(=128B)整列の時だけ**。非整列の連続
  16 ページ（0x85〜0x94, 0x90 で境界跨ぎ）を fill すると **T1=5/16 ヒット（T0 は 13/16）**＝前半が消える
  （実証 `results/iotlb_align_tb.sv` / `iotlb_align_demo.txt`）。誤ヒットせず正しくミス→walk なので機能は
  正しいが実効容量↓。coalescing は 8 整列までしか保証しないので、一般には **T0（2 ライン×8）/ T7** が妥当。
...
## T8 — 優先を 2:1 mux 直列カスケードに埋込  284 MHz / 53,787µm² / DFF1228 / 段数11
- **着想**：T5 のように「優先エンコーダ→mux」と分けず、`match[0]?spa[0]:match[1]?spa[1]:…:0` の
  **前向きネスト三項**で、**2:1 mux を 16 段直列カスケード**にして優先を mux 網に埋め込む（index0 が
  出力段、その else が index1 の mux…）。
- **格納**：T5 と同（16×72+…≈1228 DFF）。
- **lookup**：16 比較器は同じ。SPA は **16 段の 2:1 mux 依存鎖**。abc が AOI 化（o2111ai/a221oi/o21ai…）
  しても **11 段残り、FA 系で最深・最大面積**。
- **クリティカルパス**：`clkinv→o2111ai→nor4→and4→a221oi→o21ai→a22oi→o21ai→a21oi→o311a→o21ai`（11 段）。
- **観察**：段数は T5(10) より深いが、**post-place Fmax は 284>T5 246**。理由は **fanout**＝カスケードは
  各段が次段だけ駆動する低 fanout 鎖でリサイザが効く一方、T5 の優先エンコーダは「下位 OR」が高 fanout。
  **段数 ≠ Fmax（fanout が効く）**の実例。ただし両者とも T6(368) には遠く及ばず＝**優先論理を持つ限り
  T6 に勝てない**。速さの源は構造組み替えでなく**優先論理の除去（one-hot 前提）**。
