...
| T7 | line predictor（連続前提で見るラインをレジスタ決め打ち + 1 比較 validate） | ✅ | 331.0 | 34,198 | 9,119 | 844 | 13,512 | 7 | xnor2→o221ai→nor4b→and4→o211ai→or4→mux2 | ○連続 |
...
- **面積順**: **T4(12,090) ≪ T1=T3(32,572) < T7(34,198) < T0(34,556) < T2(37,868) < T6(52,874) < T5(53,466)**。
...
- **T0（現行 line baseline）は中庸**。2 ライン tag 比較（24b ×2）+ 8:1 mux で段数 7・343 MHz。整列を仮定
  しないぶん T1 より 1 比較多い。
- **T7（line predictor＝見るラインを決め打ち）は T0 とほぼ同じ（331 vs 343 MHz、むしろ微減）で、期待ほど
  速くならない**。直感（「連続なら 1 ライン分かるから 1 比較で済む」）は妥当だが、実測の理由は：
  - **クリティカルパスは tag 比較（validate）で、T7 もそれは持つ**。誤翻訳は許されない（安全性）ため、
    予測ラインでも**1 個のタグ比較で検証が必須**＝比較を経路から消せない。
  - **T0 の 2 ライン比較は元々“並列”**なので、「2 個を 1 個に減らす」ことは**段数を縮めない**（2 個の並列
    比較は 1 個と同じ深さ）。むしろ T7 は `ltag_q[cur]` の **2:1 ライン選択 mux を比較の前段に足す**ぶん
    僅かに深くなり 331<343 に。
  - 速くする条件は (a) **検証を捨てる**＝予測外れで誤 SPA（安全性違反, 不可）か、(b) **検証を次サイクルに
    パイプ化**（X3 投機 + 次段 squash）。単一サイクル安全 lookup では tag 比較が Fmax を縛る。
  - **本当の速度の源は「タグを減らす」こと**：T1（整列窓）は 16 ページを**単一の窓タグ 1 個**で表し、
    ライン毎タグも 2:1 選択も不要にして 464 MHz。決め打ち（どのラインか）ではなく、比較対象のタグ数/幅を
    減らすのが効く。
  - ただし T7 は **data を予測ライン 1 本しか読まない**（T0 は両ラインを読んで 2:1）ので、**動的電力は有利**
    （アクティブな 8:1 mux が片側だけ）。Fmax/面積では T0 同等、power では微利の位置づけ。
...
- ◯ T7 line predictor（連続前提で見るラインを決め打ち + 1 比較 validate）— T0 同等（≈343→331）。
  予測しても validate 比較が残り、並列比較の T0 に対し段数を縮められない。速さの源は「タグ数を減らす」
  T1 側にある（power だけは片側 read で微利）
