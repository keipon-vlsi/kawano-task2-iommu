...
- **T8（優先選択を前向きネスト三項で書いた版）は、実は T5 と論理的に同一**：`match[0]?spa[0]:…:0` と
  T5 の `for+if` 累算ループは**同じ lowest-index-wins 優先選択**で、**yosys 抽象ネットリストも完全に同じ
  16 $mux + 31 $eq**（確認済み）。「優先エンコーダ vs mux カスケード」は別構造ではなく、**同じ優先 mux の
  2 通りの HDL コーディング**。
  - **同一論理なのに PPA が違う**：抽象が同じでも、カスケードの順序/信号名が両者で微妙に異なり **abc の
    マッピングが別の局所解に着地** → T5 段数10/4089cell/**245.6MHz**、T8 段数11/4304cell/**284.3MHz**。
    **同じ論理を 2 通りに書くだけで Fmax +15%**＝「RTL の書き方で回路も周波数も変わる」を論理を変えずに
    示す最純粋な例（メッセージ①）。
  - **段数 ≠ Fmax**：T8 は段数が深い(11>10)のに速い(284>246)。fanout/セル選択の差。
  - **結論**：T5↔T8 の差は「優先の書き方」による abc 感度（246↔284）。**優先論理を持つ限り（T5 でも T8
    でも）T6(368) に勝てない**＝速さの源は書き方でなく**優先論理そのものの除去（one-hot, T6）**。
    ※以前 T8 を「FA 系最悪の別構造」と書いたのは不正確（T5 と同一論理）。
...
- ◯ T8 16-way FA, 優先選択を前向きネスト三項で記述 — **T5 と論理同一**（yosys 抽象 16$mux+31$eq 一致）。
  書き方違いで abc が別解に着地し 246↔284MHz（+15%, メッセージ①）。優先を持つ限り T6 に勝てない
