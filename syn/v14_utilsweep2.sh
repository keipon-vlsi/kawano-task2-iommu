...
run() {
  ...
  docker run --rm -v /space/kawano-task2-iommu:$D \
    ...
    -e MAXTRANS=0.75 -e MAXFO=16 -e SLEWM=0 -e CAPM=0 -e SETUPM=0.15 -e UTIL=$u \
    ...
    | awk -v u="$u" 'NR==1{s=$4; f=1000/(2.5-s)} /Design area/{a=$3} END{printf "UTIL%s  slack %s -> Fmax %.1f MHz  area %s um2\n", u, s, f, a}'
}
run 70
run 75
run 80
