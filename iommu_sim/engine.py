...
class Metrics:
    ...
    iotlb_hit: int = 0
    walker_busy_ns: float = 0.0     # sum of accesses*mem_latency over all walks (estimator only)
...
class Simulator:
    ...
    def _start_walk(self, t, vpn, line, is_prefetch):
        ...
        self.m.walker_busy_ns += plan.accesses * self.memory.latency   # accumulate busy time (estimator only)
