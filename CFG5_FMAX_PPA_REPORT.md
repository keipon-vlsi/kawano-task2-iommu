...
> ※ Fmax は「指定 400 MHz クロックでタイミングが閉じるか」の指標。**wire rate（持続スループット）
> を満たすかは別問題**で、§5 に示すとおり実時間で見ると周波数非依存・**パイプライン無し（~215–222 MHz）
> でも wire rate を満たす**。本節の「上限」は飽くまで到達 Fmax の話。

---

## 5. wire rate は実時間で見ると周波数非依存（パイプライン化は throughput に効かない）

§3/§4 は **Fmax**（400 MHz spec でのタイミングクロージャ）の話。だが **wire rate 要求は実時間
40.96 ns/translation**（100 GB/s ÷ 4 KB、周波数非依存）であり、これを満たすかは Fmax とは独立に
評価すべき。重要事実：**メモリレイテンシは 100 ns 固定**＝サイクル換算は周波数に比例（400 MHz で
40 cyc、222 MHz で 22 cyc）。よって低周波の設計は walk のメモリ待ちが少サイクルで済み、cyc/translation
自体が下がる。

各設計を**実周波数に MEM_LATENCY をスケールして cocotb 再測定**し、実時間を算出（`tb_coco/run_param.py`）：

| 設計 | Fmax | 周期 [ns] | MEM_LAT [cyc] | cyc/trans（実測） | **ns/trans = cyc×周期** | wire rate(≤40.96)? |
|---|---|---|---|---|---|---|
| no-pipe hd | 214.6 MHz | 4.66 | 21 | 6.43 | **29.96** | ✅ 余裕 27% |
| no-pipe hs | 222.2 MHz | 4.50 | 22 | 6.67 | **30.02** | ✅ |
| pipe hd | 266.7 MHz | 3.75 | 27 | 8.29 | **31.09** | ✅ |
| pipe hs | 395.3 MHz | 2.53 | 40 | 11.47 | **29.02** | ✅ |
| pipe @spec | 400 MHz | 2.50 | 40 | 11.47 | **28.68** | ✅ 余裕 30% |

- **実時間 ns/translation は周波数に依らず ~29–31 ns でほぼ一定**。**パイプライン無し @215–222 MHz でも
  wire rate を満たす**（~30 ns < 40.96 ns、余裕 27%）。
- **周波数が打ち消される数式**: cyc/trans はほぼ全部がメモリ待ち＝ `cyc/trans ≈ k × MEM_LAT_cyc`
  （実測 k = cyc/trans ÷ MEM_LAT = 6.43/21, 6.67/22, 11.47/40, 8.29/27 ≈ **0.29–0.31 で一定**）。
  `MEM_LAT_cyc = 100ns × freq` を代入すると `ns/trans = cyc/trans × (1/freq) = k × 100ns ≈ 29ns`
  で **freq が消える**。律速は data-leaf walk の**直列依存メモリアクセス（~2.3 往復 × 100 ns 固定）**で
  **周波数非依存**。
- **wire rate を決める真のレバー**（周波数ではない）: coalescing（1 ライン=8 翻訳に walk 償却）、PWC
  （直列 walk 段数を圧縮し往復を ~2.3 まで削減）、prefetch（1 ライン先行で 100 ns 往復を隠蔽、
  prefetch walk ~230 ns < 1 ライン消費 327 ns で追従＝これも周波数非依存）、十分な outstanding/buffer
  （Little's 則）。
- **パイプライン化（高周波化）の役割（訂正）**: **wire rate のためには不要**。唯一の理由は
  **「spec が 400 MHz クロックを固定要求している場合のタイミングクロージャ」**——400 MHz で動かすなら
  no-pipe（222 MHz 止まり）は setup 違反＝誤動作するのでパイプが要る。だがそれは「指定クロックで
  正しく動かす」ためで **throughput 向上のためではない**。本ワークロードに対し 400 MHz は過剰
  （215 MHz で wire rate 達成）。

> 注: cyc/translation は `MEM_LATENCY` を各 Fmax の実周期に合わせて（100 ns × Fmax）設定した
> cocotb 飽和駆動・steady-state 実測（`tb_coco/run_param.py`、N_REQS=296）。ns/trans は cyc×実周期。
> §1 の表の cyc/trans（11.08〜11.47）は MEM_LATENCY=40（=400 MHz 前提）固定での値で、低 Fmax 設計の
> 実時間評価にはこの §5 のスケール済み値を用いること。
